...
int map_dpp_save_config(char *param, char *value)
{
	...
	if (value)
		ret = os_snprintf(cmd, sizeof(cmd),
			...
			, DPP_KEYS_FILE, param, param, param, value, DPP_KEYS_FILE,
			param, value, DPP_KEYS_FILE);
	else
		ret = os_snprintf(cmd, sizeof(cmd),
			"sed -i '/^%s/d' %s", param, DPP_KEYS_FILE);
	...
	if (system(cmd) == -1) {
		debug(DEBUG_ERROR, "system cmd fail:%s\n", cmd);
		return -1;
	}
...
void map_dpp_save_keys(struct r3_dpp_information *dpp)
{
	...
	if (dpp) {
		if (dpp->connector)
			...
			map_dpp_save_config("connector", NULL);
		...
		if (dpp->netAccessKey) {
			...
			map_dpp_save_config("netAccessKey", NULL);
		...
		if (dpp->CsignKey) {
			...
			map_dpp_save_config("CsignKey", NULL);
	}

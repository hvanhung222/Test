			...
			if (0 == memcmp(all_agent_mac, request->dest_al_mac, ETH_ALEN)) {
				dl_list_for_each(peer, &r3_member_head, struct r3_member, entry) {
					...
					if (send_flag) {
						...
						debug(DEBUG_ERROR, "peer "MACSTR" not R3 onboarded\n"
	...
	case LIB_SET_1905_CONNECTOR:
		{
			...
			if (len == 0) {
				debug(DEBUG_ERROR, "clear connector\n");
				/* connector */
				if (dpp->connector) {
		...
		case LIB_SET_CHIRP_VALUE:
		{
			...
			debug(DEBUG_ERROR, "hash validity %d\n", p_chirp_tlv->hash_validity);
			debug(DEBUG_ERROR, "apcli mac "MACSTR"\n", MAC2STR(p_chirp_tlv->enrollee_mac));
...

{
	...
	dl_list_for_each_safe(hv_item, hv_item_nxt,
		...
		if (!os_memcmp(hv_item->hashValue, chirp_tlv->hash_payload, sizeof(hv_item->hashValue))) {
			debug(DEBUG_ERROR, "del chirp for apcli "MACSTR"\n", MAC2STR(hv_item->mac_apcli));
	...
	struct agent_radio_info *r, unsigned char radio_cnt)
{
	...
	if (ctx->renew_ctx.trigger_renew) {
		...
		if (get_top(&ctx->topo_stack, &leaf_top) == 0) {
			debug(DEBUG_OFF, " the top dev almac("MACSTR")\n",
			...
			if (!os_memcmp(al_mac, leaf_top->al_mac, ETH_ALEN)) {
				...
				debug(DEBUG_ERROR, "bss configuration renewing, wait other dev config\n");
		} else
			debug(DEBUG_ERROR, "topo stack empty\n");

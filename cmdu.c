                     ...
                     unsigned char *buffer, unsigned char *ifname)
{
	...
	debug(DEBUG_INFO, "be_to_host16(msg_hdr->message_type)(%04X) AP_AUTOCONFIG_SEARCH(%04X)\n",
	...
	if (ctx->restrict_wan_onboarding) {
		if (ctx->role != CONTROLLER) {
			if (be_to_host16(msg_hdr->message_type) == AP_AUTOCONFIG_SEARCH) {
				if ((ctx->itf[i].media_type == IEEE802_3_GROUP) && !ctx->itf[i].is_wan) {
					debug(DEBUG_ERROR, "Drop AP_AUTOCONFIG_SEARCH msg from lan interface %s\n",
						ctx->itf[i].if_name);
					return 0;
				}
			}

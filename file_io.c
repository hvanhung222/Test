	...
	unsigned char *config_bss_num)
{
	...
	if (!max_bss_num) {
		debug(DEBUG_ERROR, "input max_bss_num is 0, invaild");
		return -1;
	}

	content_len = 100 * max_bss_num;
	content = (char *)os_zalloc(content_len);
	if (!content) {
		debug(DEBUG_ERROR, "all buf fail");
		return -1;
	}
	os_memset(content, 0, content_len);
		...
		* a separator. If ssid is a \, need add \ as ESC in front of it. when parsing wts file,
		...
		if (ch == backslash) {
			...
			if (sch == space) {
				...
				ch = sch;
				if (i >= content_len) {
					debug(DEBUG_ERROR, "[%d]wts file is bigger than input buffer fail!\n", __LINE__);
					goto err;
				}
			}
		}
